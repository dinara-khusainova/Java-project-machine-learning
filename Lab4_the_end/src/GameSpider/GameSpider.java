...
public class GameSpider implements Runnable {
    ...
    //Controller controller2;//если мы захоти вернуть контроль себе
    public GameSpider(JPanel gamePanel2, JComponent componentListener) {
        ...
        //controller2 = new Controller(label2);
        background2.setBounds(0, 0, 1000, 600);
        for (int i = 0; i < 4; i++) {
            lights.add(createLights(i)); //Создается объект "Pic"

            Random rand3 = new Random();
            int randomY = rand3.nextInt((-100 - (-300)) + 1) + (-300);
            int randomX = rand3.nextInt((800 - 100) + 1) + 100;
            lights.get(i).setBounds(randomX, randomY, 40, 40);//Устанавливаются границы (положение и размер) объекта
            gamePanel2.add(lights.get(i));//добавляется на панель "gamePanel2"
        }
    ...
    void repainting(){
        for (Pic pic : lights) {
            pic.angle += 3; //крутится
            pic.repaint();//перерисовка
        }
    }
    ...
    public void run() {
        ...
        int g = 1;
        while (endd) {
            repainting();//поворот объектов и отрисовка
            g = acceleration_of_fall(i,g);   //Ускорение падения объектов

            moving_objects(g); // Перемещение объектов (вращение) если они не пересекаются с меткой playerSpider (человек-пауком)
            moving_to_closest(ai);

            i = scoring_points(i);// Если паук пересекается с объектом light по границам (касаются), увеличиваем счетчик очков и перемещаем light в случайную позицию

            // Если паук пересекается с объектом bomb (3)по границам, отображаем компонент textArea2 и устанавливаем флаг endd в false для завершения игры
            endd = crossing_with_bomb(i, endd);
            border_crossing(); // Если объекты  выходят за пределы панели gamePanel2, перемещаем их в случайные позиции
            try {
                Thread.sleep(10);
